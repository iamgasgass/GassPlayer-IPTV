...
struct SourcesView: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if displayedSources.isEmpty {
                GlassCard {
                    Text(
                        searchQuery.isEmpty
                            ? "Nessuna sorgente configurata. Tocca “Aggiungi playlist” per collegare una playlist M3U o un account supportato."
                            : "Nessun risultato per “\(searchQuery)”. Prova a cercare con un altro nome, host o tipo."
                    )
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                }
            } else if !isManualOrderingAvailable {
                ...
                VStack(spacing: 0) {
                    if contentManagement.mergedPlaylists.isEmpty {
                        ...
                        ForEach(contentManagement.mergedPlaylists) { mergedPlaylist in
                            mergedPlaylistRow(mergedPlaylist)
                            GlassRowDivider()
                        }
                    }
    ...
    private func mergedPlaylistRow(_ mergedPlaylist: MergedPlaylist) -> some View {
        HStack(spacing: 12) {
            ...
            Button(role: .destructive) {
                withAnimation(.snappy) { contentManagement.removeMergedPlaylist(mergedPlaylist) }
            } label: {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("Rimuovi \(mergedPlaylist.name)")
        }
        .padding(.vertical, 10)
    }
            ...
            GlassCard(padding: 6) {
                VStack(spacing: 0) {
                    if contentManagement.favorites.isEmpty {
                        ...
                        ForEach(Array(contentManagement.favorites.enumerated()), id: \.element.id) { index, favorite in
                            if index > 0 {
                                GlassRowDivider()
                            }
    ...
    private func sourceRow(_ source: MediaSourceConfig) -> some View {
        ...
        .contextMenu {
            if isManualOrderingAvailable {
                Button {
                    moveSource(source, direction: -1)
                } label: {
                    Label("Sposta su", systemImage: "arrow.up")
                }
                .disabled(sourceManager.sources.first?.id == source.id)
                ...
                .disabled(sourceManager.sources.last?.id == source.id)
            }
    ...
    /// Sposta una sorgente di una posizione nell'ordine reale
    /// (`sourceManager.sources`), non nell'elenco filtrato/ordinato
    /// mostrato a schermo: quando ci sono sorgenti fissate in alto,
    /// l'elenco visibile (`displayedSources`) ha un ordine diverso da
    /// quello memorizzato, quindi spostare in base agli indici visibili
    /// sposterebbe l'elemento sbagliato. `direction` è -1 per "su", +1 per
    /// "giù".
    private func moveSource(_ source: MediaSourceConfig, direction: Int) {
        guard isManualOrderingAvailable,
              let index = sourceManager.sources.firstIndex(where: { $0.id == source.id }) else {
            return
        }

        let destination = direction < 0 ? index - 1 : index + 2
        guard destination >= 0, destination <= sourceManager.sources.count else { return }

        sourceManager.move(fromOffsets: IndexSet(integer: index), toOffset: destination)
    }
