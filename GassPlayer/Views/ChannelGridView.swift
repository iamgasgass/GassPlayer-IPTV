...
struct ChannelGridView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .fullScreenCover(item: $selectedStream) { stream in
                let playlistItems = buildPlaylist()
                let initialIdx = playlistItems.firstIndex(where: { $0.id == favoriteID(for: stream) }) ?? 0
    ...
    private func buildPlaylist() -> [PlaybackItem] {
        displayedStreams.compactMap { st in
            guard let streamURL = service.streamURL(for: st, kind: kind) else { return nil }
            return PlaybackItem(
                id: favoriteID(for: st),
                url: streamURL,
                title: st.name,
                kind: kind.rawValue,
                rawItem: st
