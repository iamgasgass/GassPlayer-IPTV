...
/// Griglia canali / film / serie TV per Xtream Codes con EPG integrato,
/// caching e paginazione ottimizzata.
///
/// NOTE ARCHITETTURALI E BUG NOTI RISOLTI:
/// - Zapping canali in-place senza riapertura del player: il player modale
///   è presentato tramite `.fullScreenCover(isPresented:)` invece che `(item:)`.
///   Questo mantiene viva la stessa istanza del player durante il cambio canale
///   con i pulsanti precedente/successivo, aggiornando l'URL e avviando
///   lo stream all'istante senza animazioni di chiusura o riapertura modale.
struct ChannelGridView: View {
    ...
    @AppStorage("gassplayer.grid.density") private var channelGridDensity: String = "comfortable"
    @AppStorage("gassplayer.grid.showChannelNumbers") private var showChannelNumbers = false
    @AppStorage("gassplayer.grid.groupUIStyle") private var groupUIStyle: String = "scorrevole"
    ...
    private var isCompactGrid: Bool { channelGridDensity == "compact" }

    private var columns: [GridItem] {
        if isCompactGrid {
            return [GridItem(.adaptive(minimum: GridMetrics.compactColumnMinimum, maximum: GridMetrics.compactColumnMaximum), spacing: GridMetrics.compactColumnSpacing)]
        }
        return [GridItem(.adaptive(minimum: GridMetrics.comfortableColumnMinimum, maximum: GridMetrics.comfortableColumnMaximum), spacing: GridMetrics.comfortableColumnSpacing)]
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .fullScreenCover(isPresented: $isPlayerPresented, onDismiss: {
                ...
                if let stream = selectedStream, let url = service.streamURL(for: stream, kind: kind) {
                    PlayerView(
                        url: url,
                        title: stream.name,
                    ...
                    // `PlayerView` resta la STESSA istanza per tutta la sessione di visione.
                    // Con `.fullScreenCover(isPresented:)` invece di `.fullScreenCover(item:)`,
                    // cambiare `selectedStream` non distrugge né ri-presenta il foglio modale,
                    // ma aggiorna in-place `PlayerView` e avvia in background il nuovo flusso
                    // in modo continuo, fluido e immediato.
                    .task(id: stream.id) {
    ...
    private var content: some View {
        if kind == .series {
            ...
            ContentUnavailableView(
                ...
                description: Text(
                    selectedCategory == .all
                        ? "La sorgente non ha restituito contenuti. Prova una categoria diversa o aggiorna la sorgente."
                        : "Questa categoria non contiene elementi."
                )
            )
            .padding(.vertical, 32)
        } else {
    ...
    private func rebuildIndexIfNeeded() {
        guard indexedSourceIdentity != sourceIdentity else { return }
    ...
    private func adjacentStream(to stream: XtreamStream, offset: Int) -> XtreamStream? {
        guard let currentIndex = displayedStreams.firstIndex(where: { $0.id == stream.id }) else { return nil }
    ...
    private func loadEPGForVisibleStreams() async {
        ...
        let missingStreams = streams.filter { epgByStream[$0.streamId] == nil }
        guard !missingStreams.isEmpty else { return }

        let epg = EPGService(credentials: credentials)
        for batchStart in stride(from: 0, to: missingStreams.count, by: epgTileConcurrency) {
    ...
    private struct ChannelTile: View, Equatable {
        ...
        private var titleFont: Font { isCompact ? .caption2 : .caption }
        private var programFont: Font { .system(size: isCompact ? 8 : 9) }
        private var tileSpacing: CGFloat { isCompact ? 4 : 6 }
        private var cornerRadius: CGFloat { isCompact ? 10 : 12 }
        private var favoriteIconPadding: CGFloat { isCompact ? 5 : 7 }
        ...
        private var accessibilityLabel: String {
            guard let channelNumber else { return stream.name }
